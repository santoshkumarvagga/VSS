##############################################################
# Copyright (c) 2012-2019 Datrium, Inc. All rights reserved. #
...
class VSSProviderTest(testtypes.SystemTest):
    ...
    def upgrade_dvx_software(self):
        #deploying fixed carbon build 569 dittos, from which we upgrade to a nitrogen build 758 later.
        self.start_test('C12204')
        '''self.logger.debug('deploying fixed carbon build 569 dittos')
        self.dva.cli('/da/main/Prod/Test/dva.py -v create --testbed large_HA_dittos --jenkins http://jenkins.datrium.com/job/build-main-5.0.x/569/ /var/tmp/carbonbuild')
        self.dva.cli('/da/main/Prod/Test/dva.py -v start /var/tmp/carbonbuild')'''
        self.logger.info('Successfully deployed fixed carbon build 569')
        #get controller1 ip of carbon build dittos
        self.dva.cli('vm list skumar')
        
        self.dva.cli('vm delete skumar')
        ...
        #get controller1 ip of nitrogen build dittos
        self.dva.cli('vm list skumar')
        
        #get dvx software version of nitrogen build, by logging into it's controller1.
        dvx_version = self.dva.controllers[0].active.check_call('dvx software show')
        
        self.dva.cli('vm delete skumar')
        self.dva.controllers[0].active.check_call('entersupportmode')
        self.dva.controllers[0].active.check_call('dev root enable')
        self.dva.controllers[0].active.check_call('dev software upgrade dvx_version')
        self.logger.debug('Upgrading DVX software, Please wait a while')
        
        #upgrade will take around 20 minutes, lets take some PG APP-Consistent snaps during the time.
        #first create a PG, add VM to it.
        pgId = self.create_pg()
        ...
        time.sleep(20*60)
        self.dva.controllers[0].active.check_call('dvx software show')
        if 'status' 'success':
            self.logger.info('Successfully Upgraded DVX software')
        else:
            assert False, 'DVX software upgrade failed'
