...
class VSSProviderTest(testtypes.SystemTest):
    ...
    def upgrade_dvx_software(self):
        #deployed fixed carbon build 569 dittos, from which we upgrade to a nitrogen build 758 later.

        self.collect_ga_logs = True

        '''#Deploy a nitrogen build 758 to note down required Software version for upgrade
        self.logger.info('deploying fixed nitrogen build 758')
        self.dva.cli('/da/main/Prod/Test/dva.py -v create --testbed large_HA_dittos --jenkins http://jenkins.datrium.com/job/build-main-5.1.x/758/ /var/tmp/nitrogenbuild')
        self.dva.cli('/da/main/Prod/Test/dva.py -v start /var/tmp/nitrogenbuild')
        self.logger.info('Successfully deployed fixed nitrogen build 758')

        #get controller1 ip of dittos nitrogen build
        self.dva.cli('vm list skumar')
        file = open("testbeds.txt","r")
        mat_obj = re.compile(r'.+nitrogenbuild.+')
        reg_obj = mat_obj.search(file.read())
        logger.debug("Nitrogen build Managemnet IP - ",reg_obj.group().split()[0])
        nitrogen_mgmt_ip = reg_obj.group().split()[0]

        #get dvx software version of nitrogen build, by logging into it's controller1.
        file1 = ssh.check_output(nitrogen_mgmt_ip, 'dvx software show', username = 'admin', password = 'datrium#1')
        mat1_obj = re.compile(r'Running     ')
        reg1_obj = mat1_obj.search(file1.read())
        mat2_obj = re.compile(r'5.1.\d{3}.\d-\w{15}')
        reg2_obj = mat2_obj.search(str(reg1_obj.string))
        self.logger.debug("Nitrogen Build DVX software version is: ",reg2_obj.group())
        nitrogen_dvx_version = reg2_obj.group()'''
        ...
        '''#get controller1 ip of dittos carbon build
        self.dva.cli('vm list skumar')
        file = open("testbeds.txt","r")
        mat_obj = re.compile(r'10.80.+(.|_)+controller-01')
        reg_obj = mat_obj.search(file.read())
        print("Nitrogen build Management IP - ", reg_obj.group().split()[0])
        print(reg_obj.string)
        nitrogen_mgmt_ip = reg_obj.group().split()[0]
        print("nitrogen build ip is - ", nitrogen_mgmt_ip)'''
        ...
        #take pg snapshot every 1 minute
        self.dva.cli('protection groups schedules add %s --schedule-name "1 MIN RPO" --schedule "0 */1 * * * ?" --retention 86400' %pgId)
        #check for app consistent snaps
        self.adhoc_snap(vm, vm_summary, True, False)

        #enter into controller1 ip of carbon build dittos
        ...
        mat4_obj = re.compile(r'Success +Upgrade +.+DVX system upgraded$')
        reg4_obj = re.search(txt)

        upgrade_flag = 0
        #timeout for dvx host upgrade isgiven as 40 minutes
        for attempt in retry.retry(attempts=240, timeout=2400, sleeptime=1):
                if reg4_obj:
                    self.logger.info('Successfully Upgraded DVX software')
                    upgrade_flag = 1
                    #it means dvx host is rebooted, so we need to power on our vm
                    self.poweron_vm(vm)

                    #according to test case, PG snapshot process initiated before host reboot,
                    #should continue after powering on vm,hence verify for app consistent snaps
                    self.adhoc_snap(vm, vm_summary, True, False)
                    self.logger.info('Succesully verified for APP-Consistent PG snaps consistently before,during and after dvx software upgrade')
                    self.logger.info('Windows Uncrashed')
                    self.logger.info('Successfully verified the test case')
                else:
                    self.logger.debug('DVX software still upgrading..Please Wait!!')
		if upgrade_flag == 0:
                assert False, 'DVX software process taking longer than expected!!'
