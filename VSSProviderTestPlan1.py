...
class VSSProviderTest(testtypes.SystemTest):
    ...
    def get_last_log_backup(self, vm):
        ...
        try:
            ...
            output = ssh.check_output(self.get_guest_ip(vm),
                            "cd %s; python get_last_log_backup_time.py" % (dest_dir),
                            username='Administrator',
                            password='sha1c0w')
            return output
        except (ssh.SSHException, ssh.HostConnectError) as e:
            self.logger.debug('Expected Exception occured while executing kernel crash %s' % e)
        except Exception as e:
            self.logger.debug('Unexpected Exception occured while executing kernel crash %s' % e)
            raise
    ...
    def check_sql_log_truncation(self, vm, test_start_time, log_truncation):
        ...
        for l in o.splitlines():
            if ',' in l:
                ...
                assert dbname.strip() in ('model', 'testDB', 'testDB2')
                if log_truncation:
    ...
    def get_database_path(self, vm):
            ...
            dest_dir = "/cygdrive/c/home/bin/build/"
            for attempt in retry.retry(attempts = 3):
                try:
                    output = ssh.check_output(self.get_guest_ip(vm),
    ...
    def exch_log_truncation(self, test_type):
        ...
        self.remove_vswriter_on_guest(vm)
        self.post_test_pass('C41841')
        self.logger.info("Removing vm's, pg's and adhoc snapshots present in this vc.")
        self.delete_adhoc_snapshots()
        self.delete_pg(pgId)
        self.delete_vm(vm)
